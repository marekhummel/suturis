...
def process(data):
    ...
    if not background_running:
        mgr = mp.Manager()
        mpdict = mgr.dict()
        proc = mp.Process(target=update_param, args=(mpdict,), daemon=True)

        printt(f"Param update start (data: {type(data)})")
        background_running = True

        watcher = threading.Thread(
            target=update_param_watcher, args=(proc, data, mpdict), daemon=True
        )
        watcher.start()
...
def update_param_watcher(process, data, mpdict):
